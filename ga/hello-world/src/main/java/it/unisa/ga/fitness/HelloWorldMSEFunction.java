...
public class HelloWorldMSEFunction extends FitnessFunction<MyIndividual> {

    public HelloWorldMSEFunction() {
        super(false);
    }

    @Override
    public void evaluate(MyIndividual individual) {
        int msa = HelloWorldMSA(individual.getEncoding());
        individual.setFitness(msa);
    }

    // Calculates "Hello World!" MSA value for the given string
    private int HelloWorldMSA(String decode) {
        ...
        return squaredErrorsSum / str.length(); //Mean Squared Error
    }
}
